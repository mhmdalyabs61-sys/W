...
async def on_command_completion(ctx):
    try:
        ...
        actor_nick = actor.nick if hasattr(actor, 'nick') and actor.nick else "لا يوجد"
        actor_id = actor.id
        ...
        # التحقق الذكي من وجود شخص مستهدف (منشن) لتفادي أي أخطاء
        ...
        if isinstance(target, discord.Member):
            ...
            target_block = (
                ...
                f"• **الاسم المستعار:** `{target.nick if target.nick else 'لا يوجد'}`\n"
                f"• **الأيدي:** `{target.id}`\n"
...
async def on_message(message):
    ...
    # هذا السطر هو اللي يخلي الأوامر (مثل تايم، وسجن، وغيرها) تشتغل
    await bot.process_commands(message)
