    ...
    if channel:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"بفضلك صرنا \n"
                f"**{member_count}** عضو!\n\n"
                f"لا تنسى تشيك على <#1543129300377796631>\n\n"
...
# ==================== إعدادات السجن ====================
JAIL_CHANNEL_ID = 1543129260032663665
JAIL_ROLE_ID = 1543292685224378500
...
AZKAR_LIST = [
    ...
    "❤️ | **سُبحان الله، والحمدُ لله، ولا إله إلا الله، واللهُ أكبر.**",
    "💎 | **لا حول ولا قوة إلا بالله العلي العظيم.**",
    "🌹 | **اللهم صل وسلم على نبينا محمد وعلى آله وصحبه أجمعين.**",
    "🌸 | **سُبحان الله وبحمده عدد خلقه، ورضا نفسه، وزنة عرشه، ومداد كلماته.**",
    ...
    "🌙 | **أَصْـبَحنا وَأَصْـبَح المُلْكُ لله، والحمدُ لله، لا إله إلا اللهُ وحدَهُ لا شريكَ له.**",
    "🛡️ | **بِسْمِ اللَّهِ الَّذِي لَا يَضُرُّ مع اسمِهِ شيءٌ في الأرضِ ولَا في السماءِ وهو السَّميعُ العليم.**",
...
async def on_ready():
    try:
        ...
        print(f"تم بنجاح مزامنة {len(synced)} من أوامر السلاش.")
    except Exception as e:
...
async def set_azkar_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    AZKAR_CHANNELS[guild_id] = channel.id
...
# مجموعة حفظ الكلمات المحظورة
BANNED_WORDS = set()

# 1. حدث الحذف الفوري والصامت
async def setup_banned_words(bot):
    @bot.event
...
# 2. مجموعة أوامر السلاش لإدارة الكلمات المحظورة
badwords_group = discord.app_commands.Group(name="الكلمات_المحظورة", description="إدارة نظام الكلمات المحظورة وحذفها تلقائياً")

@badwords_group.command(name="إضافة", description="إضافة كلمة جديدة إلى قائمة الكلمات المحظورة")
@discord.app_commands.describe(word="الكلمة المراد حظرها")
@discord.app_commands.checks.has_permissions(administrator=True)
async def add_badword(interaction: discord.Interaction, word: str):
    clean_word = word.strip().lower()
    if clean_word in BANNED_WORDS:
        await interaction.response.send_message(f"⚠️ | الكلمة **({clean_word})** موجودة مسبقاً في القائمة المحظورة!", ephemeral=True)
    else:
        BANNED_WORDS.add(clean_word)
        await interaction.response.send_message(f"✅ | تم بنجاح إضافة **({clean_word})** إلى قائمة الكلمات المحظورة.", ephemeral=True)

@badwords_group.command(name="إزالة", description="إزالة كلمة من قائمة الكلمات المحظورة")
@discord.app_commands.describe(word="الكلمة المراد إزالتها")
@discord.app_commands.checks.has_permissions(administrator=True)
async def remove_badword(interaction: discord.Interaction, word: str):
    clean_word = word.strip().lower()
    if clean_word in BANNED_WORDS:
        BANNED_WORDS.remove(clean_word)
        await interaction.response.send_message(f"✅ | تم بنجاح إزالة **({clean_word})** من القائمة المحظورة.", ephemeral=True)
    else:
        await interaction.response.send_message(f"❌ | الكلمة **({clean_word})** غير موجودة في القائمة أصلاً!", ephemeral=True)

@badwords_group.command(name="عرض", description="عرض جميع الكلمات المحظورة حالياً في السيرفر")
@discord.app_commands.checks.has_permissions(administrator=True)
async def list_badwords(interaction: discord.Interaction):
    if not BANNED_WORDS:
        await interaction.response.send_message("📌 | لا توجد أي كلمات محظورة مضافة حالياً.", ephemeral=True)
    else:
...
# لتسجيل الأوامر (تأكد تضيف `bot.tree.add_command(badwords_group)` عندك في البوت)
